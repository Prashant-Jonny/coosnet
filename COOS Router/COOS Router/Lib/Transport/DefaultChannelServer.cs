...
#endif
#if NET
        //Info : http://stackoverflow.com/questions/1273139/c-java-hashmap-equivalent
        private Dictionary<string, RouterChannel> channelMappings = new Dictionary<string,RouterChannel>();
#endif
...
#endif
#if NET
        public virtual void setChannelMappings(Dictionary<string, RouterChannel> channelMappings)
#endif
        {
            this.channelMappings = channelMappings;
        }

        public virtual void addChannelMapping(string uuid, RouterChannel channel)
        {
            this.channelMappings.Add(uuid, channel);
        }


        public virtual void setCOOSInstance(COOS coosInstance)
        {
            this.coosInstance = coosInstance;
        }


        public virtual void setLinkManager(ILinkManager linkManager)
        {
            this.linkManager = linkManager;
        }

        public virtual void initializeChannel(ITransport transport)
        {
...
{
    	 //base.setProperties(addedProperties);
...

{
        ...
        /// <summary>
        /// Process only connect message and give uuid to endpoint
        /// </summary>
        ...
        public override void processMessage(IMessage msg)
        {
            ...
            if (msg.getHeader(IMessagePrimitives.MESSAGE_NAME).Equals(DefaultChannel.CONNECT))
            {
                ...
                try
                {
                    ...
                    Link channelInLink = channel.getInLink();
                    if (channelInLink == null)
                        logger.warn("Got null in channel inlink!");

                    transport.setChainedProcessor(channelInLink);
                    channel.getInLink().setChainedProcessor(linkManager.getDefaultProcessor()); // The router
                    ...
                    if (props != null)
                    {
                        ...
                        if (props.ContainsKey(Link.ALIASES))
                        {
                            Type aliasType = props[Link.ALIASES].GetType();
                            if (aliasType == typeof(ArrayList))
                            {
                               
                               
                                foreach (string alias in (ArrayList)props[Link.ALIASES])
                                    aliases.Add(alias);
                            } else if (aliasType == typeof(List<string>)) 
                            
                              aliases = new List<string>((List<string>)props[Link.ALIASES]);
                            else
                                logger.info("Alias type: " + aliasType.ToString());
                        }
                ...
                {
                    logger.warn("DefaultChannelServer.processMessage(...). Exception caught. Removing link " + conUuid, e);
