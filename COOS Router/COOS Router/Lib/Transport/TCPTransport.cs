...
using java.lang;
using System.Collections.Generic; // IRunnable interface compability
...
namespace Org.Coos.Messaging.Transport
{
    ...
    public class TCPTransport : DefaultProcessor, ITransport, IService
    {
       ...
       protected TCPTransportManager transportManager;
        ...
        /// </summary>
        static protected List<IMessage> mailbox = new List<IMessage>();
...
#if LOGGING
        private static Org.Coos.Messaging.Util.ILog logger = LogFactory.getLog(typeof(TCPTransport).FullName);
#endif
        ...
        public TCPTransport(Socket socket, TCPTransportManager tm) {
            this.socket = socket;
        ...
        public void start()
        {
                ...
                // This case happens when TCPTransport is started by TCPTransportManager
                // socket will be the accepted server socket for communication with plugin/other routers
                if (socket != null)
                {
                ...
                var connectTask = Task.Factory.StartNew(() => connectToRouter(retryStr,ct),ct);
        ...
        /// <returns></returns>
        private void connectToRouter(string retryStr,CancellationToken ct)
        {
            try
            {

                // Info : http://msdn.microsoft.com/en-us/library/dd997396.aspx
    ...
    public class Reader : IRunnable
    {
        ...
        private readonly ILog logger = LogFactory.getLog(typeof(Reader).FullName);
        ...
        public Reader(Socket socket,Stream incomingStream, IChannel channelReader,  bool retry, int retryTime)
        {
            ...
            var readerTask = Task.Factory.StartNew(() => this.run());
#if NOTASK
        ...
        public virtual void run()
        {
            try
            {
                ...
                while (running)
                {
                        ...
                        if (socket.Poll(30*1000000, SelectMode.SelectRead))
                            ...
                            // if (networkStream.DataAvailable)
                            {
                               // Debug.Print("Reader: " + socket.Available.ToString() + " bytes received. Deserializing...");
                                ...
                                logger.info("Reader: received msg.: " + msg.getName());

                                if ((msg.getSerializedBody() == null) ||
                                   ((msg.getSerializedBody() != null) &&
                                    (msg.getSerializedBody().Length <= MAX_BODY_LENGTH)))
                                {
                                    if (TCPTransport.transportProcessor != null)
                                        TCPTransport.transportProcessor.processMessage(msg);
                                    else
                                        logger.error("Reader: No transport processor for further message processing");
                                }
                                else
                    ...
                    {
                        //e.printStackTrace();
                        logger.error("Error. Aborting reader.",e);
                        running = false;
                        failed = true;
                    }
                }

                logger.info("Reader: Closing network stream.");
    ...
    public class Writer : IRunnable
    {
        ...
        Socket socket;
        private readonly ILog logger = LogFactory.getLog(typeof(Writer).FullName);

        public Writer(Socket socket,Stream networkStream, IChannel channelWriter, List<IMessage> mailbox, AutoResetEvent msgInMailBoxEvent)
        {
            ...
            this.socket = socket;
            var writerTask = Task.Factory.StartNew(() => this.run());
#if NOTASK
            ...
            try
            {
                ...
                while (running)
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            byte[] serialMsg = msg.serialize();

                            //Debug.Print("Writer sending: " + msg.getName());
                            logger.info("Writer sending: " + msg.getName());
                            ...
                            //{
                                outgoingStream.Write(serialMsg, 0, serialMsg.Length);
