...
public abstract class DefaultEndpoint : DefaultProcessor, IEndpoint {
    ...
    private Dictionary<string,IProcessor> outLinks = new Dictionary<string,IProcessor>();
    ...
    /// </summary>
    private Dictionary<string,IAsyncCallback> callbacks = new Dictionary<string,IAsyncCallback>();
    ...
    protected List<IMessage> deferQueue = new List<IMessage>();
#if LOGGING
   // JAVA protected ILog log = LogFactory.getLog(this.getClass().getName());
     ...
     public override void setName(string name) {
        ...
        if ((name != null) && !UuidHelper.isUuid(name)) {
            if (!aliases.Contains(name))
                ...
                log.warn("DefaultEndpoint.setName, tried to add alias that exsists :" + name);
        }
    ...
    public override void setProperties(Dictionary<string,string> properties) {
        ...
        string timoutStr = null;
        if (properties.ContainsKey(IEndpointPrimitives.PROP_EXCHANGE_TIMEOUT))
          timoutStr = (string)properties[IEndpointPrimitives.PROP_EXCHANGE_TIMEOUT];
   ...
   // JAVA protected IProcessor resolveOutgoingProcessor(string protocol) throws EndpointException {
    ...
    private void processTypeMessage(string exchangeId, IMessage msg, IMessage retMsg, ExchangePattern exchangePattern, IConsumer consumer, string exchangePatternStr)
{
     // Message handling
                IExchange exchange = null;

                if ((exchangeId != null) && exchanges.ContainsKey(exchangeId) && !exchangePattern.isOutBoundInitiated())
                {

                   
                    // this is a return or a message to same endpoint.
                    IExchange exchange2 = (IExchange) exchanges[exchangeId];
                    bool result =  exchanges.Remove(exchangeId);
#if LOGGING
                    ...
                    if (callbacks.ContainsKey(exchangeId))
                    {
                        // If this exchange is to be processed async., then call callback on thread
                        processCallbackForExchange(exchangeId, exchange2);
                ...
                } else if (consumer != null)
                {
                    ...
                    if (msg.getHeader(IMessagePrimitives.MESSAGE_NAME).Equals(EdgeLCMMessageFactory.EDGE_REQUEST_STATE)) {
...
#endif
#if TASK
                        var LCMEdgeEquestTask = Task.Factory.StartNew(() => {
                            try {
                                LCMReport.reportState();
                            } catch (Exception e)
                            {
                                log.warn(
                                            "Exception caught by processMessage(). Message " +
                        ...
                        return;
                    } else if (msg.getHeader(IMessagePrimitives.MESSAGE_NAME).Equals(EdgeLCMMessageFactory.EDGE_REQUEST_CHILD_STATE)) {
...
#endif
#if TASK
                         var LCMEReportChildStateTask = Task.Factory.StartNew(() =>
                         {
                             try
                             {
                                 LCMReport.reportChildState(childAddress);
                             }
                             catch (Exception e)
                             {
                                 log.warn(
...
#if TASK
                            var LCMReportChildenTask = Task.Factory.StartNew(() =>
                            {
                                try
                                {
                                    LCMReport.reportChildren();
                                }
                                catch (Exception e)
                                {
                                    log.warn(
                    ...
                    if (exchangePatternStr.Equals(ExchangePattern.OutIn)) {
                        ...
                        if (exchangePatternStr.Equals(ExchangePattern.RobustOutOnly))
                        {
                            processExchangeDeliverySuccess(msg);
                        }

                        exchange = createExchange(new ExchangePattern(ExchangePattern.InOnly));
                    }

                    processMessageInConsumer(exchangeId, msg, consumer, exchange);

                }
  }

    private void processExchangeDeliverySuccess(IMessage msg)
    {
        ...
        IExchange exchangeDeliverySuccess = createExchange(new ExchangePattern(ExchangePattern.RobustInOnly));
        exchangeDeliverySuccess.setExchangeId(msg.getHeader(IMessagePrimitives.EXCHANGE_ID));
        confMsg.setReceiverEndpointUri(msg.getHeader(IMessagePrimitives.ROBUST_DELIVERY_ACK_URI));
        exchangeDeliverySuccess.setOutBoundMessage(confMsg);
#if LOGGING

        log.debug("Endpoint: " + uri + ", Sending robust ack: " + exchangeDeliverySuccess);
#endif
        processExchange(exchangeDeliverySuccess);
        #endregion
    }

    private void processMessageInConsumer(string exchangeId, IMessage msg, IConsumer consumer, IExchange exchange)
    {
...
#endif
#if TASK
            // Uses .NET 4.0 Parallel Task -> optimized for concurrency on multicore processors
    ...
    private void processCallbackForExchange(string exchangeId, IExchange exchange2)
    {
        ...
        var callbackTask = Task.Factory.StartNew(() =>
        {
            ...
            lock (callbacks) // Mutually exclusive thread access to callbacks
            {
                callback = (IAsyncCallback)callbacks[exchangeId];
                callbacks.Remove(exchangeId);
            }
            callback.processExchange(exchange2);
        });
#endif
    ...
    public override void processMessage(IMessage msg) {
       ...
       // if (msgType.Equals(IMessagePrimitives.TYPE_MSG) || msgType.Equals(IMessagePrimitives.TYPE_ERROR)) {
            ...
            string exchangePatternStr = msg.getHeader(IMessagePrimitives.EXCHANGE_PATTERN);
            ExchangePattern exchangePattern = new ExchangePattern(exchangePatternStr);
             string exchangeId = msg.getHeader(IMessagePrimitives.EXCHANGE_ID);
            ...
            if (errorMsg)
                processErrorMsg(exchangeId, retMsg);
            else if (typeMsg)
                processTypeMessage(exchangeId, msg, retMsg, exchangePattern, consumer, exchangePatternStr);
...
log.error("Exception caught while stopping endpoint.", e);
    ...
    public virtual void addAlias(string alias)
    {
        ...
        if ((alias != null) && !UuidHelper.isUuid(alias) && UuidHelper.isValidAliasForUuid(alias, getEndpointUuid())) {
            aliases.Add(alias);
            updateAliases();
        } else {
#if LOGGING
    ...
    public virtual void setLinkAliases(List<string> regAliases, Link outlink)
    {
        // Not implemented
    }
